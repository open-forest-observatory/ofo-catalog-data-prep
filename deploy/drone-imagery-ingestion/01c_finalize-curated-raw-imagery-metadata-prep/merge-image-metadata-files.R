# Purpose: Take all the post-curation image-level metadata files in
# POST_CURATION_PARSED_EXIF_FOR_RETAINED_IMAGES_PATH and combine them into a single file that makes
# it easier to filter and intersect image metadata. NOTE: This assumes that all the image metadata
# files that are potentially relevant are in the directory
# POST_CURATION_PARSED_EXIF_FOR_RETAINED_IMAGES_PATH. This shouldn't be assumed, and therefore if
# you're not sure, you may wish to download them to this folder from the object store.
...
metadata_files = list.files(POST_CURATION_PARSED_EXIF_FOR_RETAINED_IMAGES_PATH, pattern = "_image-metadata.gpkg$", full.names = TRUE)
...
# Write
st_write(metadata_sf, POST_CURATION_FULL_METADATA_PER_IMAGE_COMBINED_FILEPATH, delete_dsn = TRUE)
